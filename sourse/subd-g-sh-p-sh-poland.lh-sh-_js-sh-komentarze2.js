(function($) {
	$.fn.favoriteArticleButton = function(articleId) {
		...
		$.get('?ajax=1&action=get_favorite_article_info&articleId='+articleId, function(data) {
	  			if (data == 'logout') {
	  				var location = '?ajax=1&action=get_login_form&height=262&width=427&forwardUrl=http://poland.lh/profil/dodaj-artykul-do-schowka/'+articleId;
	  			...
	  			else
	  			{
	  				...
	  				{
	  					var location = 'http://poland.lh/profil/dodaj-artykul-do-schowka/'+articleId;
...
function ArticleComments(params) {
	...
	$.ajax({
		...
		success: function(json) {
			...
			//Nagłówek komentarzy
			_this.header = $('<div class="commentsHeader"><button type="button"><img src="http://poland.lh/subd-g/p/poland.lh/prawo/_i/skomentuj_button.gif"></button><span class="countInfo">Liczba komentarzy: '+json.thread.post_count+'</span></div>');
	...
	this.showComments = function() {
		...
		if (this.thread.url) {
			$('<a class="threadLink" alt="zobacz całą dyskusję" title="Zobacz całą dyskusję na forum" href="'+this.thread.url+'"><img src="http://poland.lh/subd-g/p/poland.lh/prawo/_i/zobacz_cala_dyskusje.png"></a>').appendTo(this.params.rootElement);
		}
	};
	...
	this.buildNoCommentsInfo = function() {
		...
		$('<div class="center padd5" style="padding-top: 10px; background: #E3E3E3">Ten artykuł nie był jeszcze skomentowany</div><div class="center" style="font-size: 18px; padding-top: 25px; height: 130px; background: #E3E3E3 url(http://poland.lh/subd-g/portal/poland.lh/kobieta/chmorka.png) no-repeat 50% 0;" ><span>Podziel się z nami<br />swoją opinią</span><br /><br /><span class="link" style="color: white;">Rozpocznij dyskusję</span></div><div class="center" style="height: 40px; background: #E3E3E3 url(http://poland.lh/subd-g/portal/poland.lh/kobieta/chmorka.png) no-repeat 50% -155px;" >lub<br />Sprawdź gdzie prowadzone są najciekawsze rozmowy</div>').appendTo(mainElement);
	...
	this.getAvatar = function(fileName, width, height) {
		//Domyślny avatar
		var obj = {
			url: 'http://poland.lh/subd-g/p/poland.lh/biznes/_i/ico-guest.png',
			width: 50,
			height: 50
		};
		if (fileName && width && height) {
			...
			obj.url = 'http://poland.lh/subd-g/wieszjak/image/avatar/' + fileName;
		}
